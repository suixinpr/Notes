...
与 RegularLock 锁相关的全局变量有三个：

- **LockMethodLockHash**：指向的哈希表为 `LOCKTAG -> LOCK`，存储在**共享内存**中。
- **LockMethodProcLockHash**：指向的哈希表为 `PROCLOCKTAG -> PROCLOCK`，存储在**共享内存**中。
- **LockMethodLocalHash**：指向的哈希表为 `LOCALLOCKTAG -> LOCALLOCK`，对于每一个后端都是**本地**的。

下面来介绍一下这几个锁相关的结构体：

- `LOCK` 存在于**共享内存**中，用来保存当前数据库中所有事物的锁对象，被称为**主锁表(LockMethodLockHash)**
- `PROCLOCK` 存在于**共享内存**中，用来保存当前进程（会话 -- backend）的事务锁状态，用于建立锁和进程会话的关系。
- `LOCALLOCK` 存储到后端本地——**非共享**，因为实际操作过程中同一个事务可能会在一个锁对象上申请多次相同类型的锁，这样就没有必要做冲突检测，直接从会话本地拿就好了。

除此之外还有一个全局变量 **FastPathStrongRelationLocks**，保存着快速路径强关系锁相关的信息。
